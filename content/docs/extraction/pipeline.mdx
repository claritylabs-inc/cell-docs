...
CL-0 SDK's extraction pipeline processes insurance documents in multiple passes, producing structured data with page-level provenance.
...
```typescript
import { extractFromPdf, applyExtracted } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { extractSectionsOnly } from "@claritylabs-inc/cl-sdk";
...
Documents are split into page chunks for section extraction. CL-0 SDK uses an adaptive strategy:
...
```typescript
import { getPageChunks } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { mergeChunkedSections, mergeChunkedQuoteSections } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { stripFences } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { sanitizeNulls } from "@claritylabs-inc/cl-sdk";
