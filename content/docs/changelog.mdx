...
This release contains maintenance updates and internal improvements with no breaking changes to the public API.
All notable changes to `@claritylabs-inc/cl-sdk` are documented here.
...
- `AgentContext.agentName` is now optional — defaults to `"CL-0 Agent"` if not set, replacing the previously hardcoded `"Clarity Agent"`
...
Initial release of `@claritylabs-inc/cl-sdk`.
