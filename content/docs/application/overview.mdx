...
CL-0 SDK provides prompt builders for every stage of insurance application processing — from detecting application forms to filling completed PDFs.
...
```typescript
import { buildFieldExtractionPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { buildAutoFillPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { buildQuestionBatchPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { buildAnswerParsingPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { buildBatchEmailGenerationPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { buildReplyIntentClassificationPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { buildFieldExplanationPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { buildConfirmationSummaryPrompt } from "@claritylabs-inc/cl-sdk";
...
<Tabs items={["AcroForm (fillable PDFs)", "Flat PDFs"]}>
  <Tab value="AcroForm (fillable PDFs)">
    ```typescript
    import { buildAcroFormMappingPrompt } from "@claritylabs-inc/cl-sdk";
  ...
  <Tab value="Flat PDFs">
    ```typescript
    import { buildFlatPdfMappingPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { buildLookupFillPrompt } from "@claritylabs-inc/cl-sdk";
