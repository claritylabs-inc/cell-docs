...
CL-0 SDK exports Claude `tool_use`-compatible tool schemas for common insurance agent operations. These are **schema-only** — CL-0 SDK provides the definitions, and consumers implement the execution logic.
...
```typescript
import { DOCUMENT_LOOKUP_TOOL } from "@claritylabs-inc/cl-sdk";
```
...
```typescript
import { COI_GENERATION_TOOL } from "@claritylabs-inc/cl-sdk";
```
...
```typescript
import { COVERAGE_COMPARISON_TOOL } from "@claritylabs-inc/cl-sdk";
```
...
```typescript
import { AGENT_TOOLS } from "@claritylabs-inc/cl-sdk";
...
```typescript
import { AGENT_TOOLS, buildAgentSystemPrompt } from "@claritylabs-inc/cl-sdk";
...
```typescript
import type { ToolDefinition } from "@claritylabs-inc/cl-sdk";
